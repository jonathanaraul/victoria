...
function validaIndices(validacion) {
	 if(validacion == false) {
			$('#mensajeajax-negativo').empty();
			$('#mensajeajax-negativo').html('<button type="button" class="close" data-dismiss="alert"><i class="icon-remove"></i></button><strong> <i class="icon-remove" ></i> Error </strong> Debe seleccionar por lo menos un tipo de estudio.<br>');
...
function validaCamposLlenos(elementos) {
	...
	$.each(elementos, function(indice, valor) {
		...
		if($(valor).hasClass('textarea')) {
			campo = $.trim($(valor).prev().html());
		} else {
			campo = $.trim($(valor).parent().prev().html());
			if(campo == '')
				campo = $.trim($(valor).parent().parent().prev().html());
		}
...
function validaCamposNumericos(elementos) {
	...
	$.each(elementos, function(indice, valor) {
		...
		if(!isNumber(auxiliar)) {
			$('#mensajeajax-negativo').empty();
			$('#mensajeajax-negativo').html('<button type="button" class="close" data-dismiss="alert"><i class="icon-remove"></i></button><strong> <i class="icon-remove" ></i> Error </strong> El valor del campo ' + campo + ' debe ser numerico.<br>');
			$('#mensajeajax-negativo').css('display', 'block');
			validacion = false;
			return false;
		} else {
